...
#        self.history_log.to_csv(file_path + self.logname, mode='w')
    ...
    def send(self):
        ...
        if text == "help":
            ...
            msg = ("h: recall previously typed messages.<br>\n"
                   ...
                   "u: update all parameter values.<br>\n"
        ...
        # command format: scan (det1 det2 ...;) scan_param begin end step dwell
    ...
    def check_param_scan(self, text):
        ...
        # input text format: scan det1 det2 ....: scan_param begin end step dwell
        ...
        if text[c+1:].find(':') == -1 and space == 5:
            ...
            if (sptext[0] != '') and (len(sptext) == 5):
                ...
                check_msg = ("format: scan [det1 det2 .. :] parameter begin end step well_time")
...
class SpectrumWidget(QWidget):
    ...
    def scan_loop(self, det, sptext):
        ...
        det = det[:min(len(det),5)] # maximum number of curves to be plotted is 5.



        # sptext a string has a format of "parameter begin end step well_time"

        # det is a string containing parameters for plotting
        ...
        scan_data = [] # the 1st data point of measurement #1
        ...
        for j in range(len(det)):
            ...
            self.curve.append(self.plotWidget.plot(scan_x, scan_data, pen=color[j], linewidth=2))
        ...
        for i in range(n+1):
            ...
            for j in range(len(det)):
                ...
                cc=0.8**(j)*100*math.sin((10*j+1)*(scan_x[i]-(x1+x2)/2)/2) # math.cos(scan_x[i]/100) #get numeric data:  assuming  sin function

                counts.append(cc)

                print("cc=  ", cc, 'counts  = ', counts)

                param[det[j]]=cc



            # plot data and show in status and command area

            #param[det] = counts # detector

            print('i=',i, scan_x[i], param[det])

            scan_data.append(float(param[det])) #list

            #self.curve.setData(scan_x, scan_data)

            status_widget_global.show_text()

            QtGui.QApplication.processEvents()

            cmd_global.sysMsg('i = ' +str(i)+'   '+scan_param+' = '+ str(scan_x[i])+ ',  '+str(param[det]))
